...
namespace SevenWonders.WebAPI.Controllers
{
    public class SearchController : ApiController
    {
                ...
                foreach (var tempCityTo in citiesTo)
                {
                    if (tempCityFrom.Id != tempCityTo.Id)
                    {
                            ...
                            && availableRooms != null && availableRooms.Count > 0)
                        {
                            ...
                            var hotelForTours = availableRooms.GroupBy(x => x.Room.HotelId).ToDictionary(x => x.Key, x => x.OrderBy(y => y.Room.Price));
                            foreach (var hotelModel in hotelForTours)
                            {
                                ...
                                var newTour = new TourForSearchModel()
                                {
                                    ...
                                    Rooms = hotelModel.Value
                                        .GroupBy(x=>x.RoomId)
                                        .ToDictionary(x => x.Key, x => x)
                                        .Select(x => convertToRoomShortInfoModel(x.Value))
                                        .ToList()
                            };
        ...
        public IHttpActionResult BookTour([FromBody] TourForBookingModel model)
        {
            try
            {
                RoomItem roomItem = db.RoomItems.Find(model.RoomId);
                ...
                decimal totalPrice
                    = model.Duration * (roomItem.Room.Price + model.PersonAmount * (model.WithoutFood ? 0 : roomItem.Room.Hotel.FoodPrice))
                    + model.PersonAmount * (leaveSchedule.Flight.Price + returnSchedule.Flight.Price);
                Reservation reservation = new Reservation()
                {
                    RoomItemId = model.RoomId,
        ...
        private List<RoomItem> availableRooms(int cityId, int people, DateTime departureDate, int duration)
        {
            ...
            var allRooms = db.RoomItems.Where(x => x.Room.Hotel.CityId == cityId
                                            && x.Room.MaxPeople >= people
                                            && !x.IsDeleted
                                            && !x.Room.IsDeleted
                                            && !x.Room.Hotel.IsDeleted).ToList();

            var availableRooms = new List<RoomItem>();
            availableRooms = allRooms
                .Where(x => !reservations.Any(y =>y.RoomItem.Id == x.Id && !(departureDate >= y.ReturnDate) && !(arrivalDate <= y.LeaveDate))).ToList();
        ...
        private RoomShortInfoModel convertToRoomShortInfoModel(IEnumerable<RoomItem> roomItems)
        {
            var room = roomItems.First()?.Room;
            return new RoomShortInfoModel()
            {
                Id = roomItems.First().Id,
                CountOfRooms = roomItems.Count(),
                RoomType = room.RoomType.Name,
                MaxPeople = room.MaxPeople,
                Price = room.Price,
                WindowView = room.WindowView,
                Equipments = room.Equipments.Select(x => x.Name).ToList(),
                RoomPhotos = room.RoomPhotos.Select(x => new PhotoModel() { Id = x.Id, PhotoLink = x.PhotoLink }).ToList()
            };
        }
    }
}
